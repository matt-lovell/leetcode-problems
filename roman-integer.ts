...
var romanToInt = function(s: string) {
    ...
    sArray.forEach((char, index) => {
            ...
            if (char === 'I') {
                ...
                if(nextChar !== 'V' && nextChar !== 'X') total += charValue(char)
                // Else subtract the value from total
                else total -= charValue(char)
            }
            // Repeat for other 2 characters
            else if (char === 'X') {
                const nextChar = sArray[index + 1]
                if(nextChar !== 'L' && nextChar !== 'C') total += charValue(char)
                else total -= charValue(char)
            }
            else if (char === 'C') {
                const nextChar = sArray[index + 1]
                if(nextChar !== 'D' && nextChar !== 'M') total += charValue(char)
                else total -= charValue(char)
            }
            else total += charValue(char)
        }
